...
def load_result_directory(dir_path: str) -> Dict:
    """加载批处理结果目录并计算正确率"""
    
    # 检查必要的子目录
    images_dir = os.path.join(dir_path, 'images')
    detect_images_dir = os.path.join(dir_path, 'detect_images')
    real_images_dir = os.path.join(dir_path, 'real_images')
    detect_labels_dir = os.path.join(dir_path, 'detect_labels')
    real_labels_dir = os.path.join(dir_path, 'real_labels')
    
    if not os.path.exists(images_dir):
        return {'error': f'images目录不存在: {images_dir}'}
    ...
    for img_name in images:
        ...
        # 解析标签
        detect_label_path = os.path.join(detect_labels_dir, f'{base_name}.txt')
        real_label_path = os.path.join(real_labels_dir, f'{base_name}.txt')
        
        detections = parse_yolo_label(detect_label_path, img_w, img_h)
        ground_truths = parse_yolo_label(real_label_path, img_w, img_h)
        
        # 计算正确率
        accuracy = calculate_accuracy(detections, ground_truths)
        
        # 检查相关文件是否存在
        has_detect_img = os.path.exists(os.path.join(detect_images_dir, img_name))
        has_real_img = os.path.exists(os.path.join(real_images_dir, img_name))
        ...
        results.append({
            ...
            'detection_count': len(detections),
            'gt_count': len(ground_truths),
            'has_detect_img': has_detect_img,
            'has_real_img': has_real_img,
            'width': img_w,
            'height': img_h
        })
    ...
    # 统计正确率分布
    acc_dist = {
        '>=98%': sum(1 for r in results if r['accuracy'] >= 0.98),
        '95-98%': sum(1 for r in results if 0.95 <= r['accuracy'] < 0.98),
        '90-95%': sum(1 for r in results if 0.90 <= r['accuracy'] < 0.95),
        '85-90%': sum(1 for r in results if 0.85 <= r['accuracy'] < 0.90),
        '<85%': sum(1 for r in results if r['accuracy'] < 0.85),
    }
    
    # 统计正/负样本
    positive_count = sum(1 for r in results if r['gt_count'] > 0)
...
@app.route('/api/load_result_dir', methods=['POST'])
def api_load_result_dir():
    """加载批处理结果目录"""
    global result_dir_cache, image_manager
    
    data = request.json
    dir_path = data.get('dir_path', '').strip()
    
    if not dir_path:
        return jsonify({'code': 400, 'message': '请输入目录路径'})
    
    # 处理路径
    dir_path = os.path.normpath(dir_path)
    
    if not os.path.exists(dir_path):
        return jsonify({'code': 404, 'message': f'目录不存在: {dir_path}'})
    
    if not os.path.isdir(dir_path):
        return jsonify({'code': 400, 'message': '路径不是目录'})
    
    # 加载并计算结果
    print(f"[INFO] 加载结果目录: {dir_path}")
    results = load_result_directory(dir_path)
    
    if 'error' in results:
        return jsonify({'code': 400, 'message': results['error']})
    
    # 更新image_manager指向images子目录
    images_dir = os.path.join(dir_path, 'images')
    image_manager = ImageFolderManager(images_dir)
    
    # 缓存结果
    result_dir_cache = results
    
    print(f"[INFO] 加载完成: {results['total_count']} 张图片, 平均正确率: {results['avg_accuracy']*100:.1f}%")
    ...
    return jsonify({
        ...
        'data': {
            ...
            'accuracy_distribution': results['accuracy_distribution'],
...
@app.route('/api/get_result_image', methods=['POST'])
def api_get_result_image():
    """获取结果目录中单张图片的对比视图"""
    global result_dir_cache
    
    if not result_dir_cache:
        return jsonify({'code': 400, 'message': '请先加载结果目录'})
    
    data = request.json
    image_name = data.get('image_name')
    
    if not image_name:
        return jsonify({'code': 400, 'message': '缺少图片名称'})
    
    dir_path = result_dir_cache['dir_path']
    ...
    detect_img_path = os.path.join(detect_images_dir, image_name)
    real_img_path = os.path.join(real_images_dir, image_name)
    ...
    orig_b64 = encode_image(img_path)
    detect_b64 = encode_image(detect_img_path)
    real_b64 = encode_image(real_img_path)
    
    if not orig_b64:
        return jsonify({'code': 400, 'message': '无法读取图片'})
